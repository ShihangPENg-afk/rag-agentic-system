...
> 适用版本：rag-agentic-system（含 PostgreSQL 混合持久化与工业预测联动）  
...
1. 在终端 B **停止并重新启动** `make run`（或 `docker compose restart rag-agentic-system`）。  
...
3. 结束语：完整 API 与 Docker 部署见 README；LoRA 微调**尚未接入 rag-agentic-system**，生成仍走 DashScope `qwen-plus`。
...
`rag-agentic-system-ui-demo-2026-06-11.mp4`

可与 [delivery_checklist.md](delivery_checklist.md) 中 UI 与 PostgreSQL 验收项一并归档（录屏文件见 README 网盘链接，不提交 Git）。
